// Utilitários para manipulação de arquivos - VERSÃO CORRIGIDA
...
export const validateFile = (file, maxSizeInMB = null) => {
  ...
  if (file.type.startsWith('video/') && file.size > 200 * 1024 * 1024) {
    console.warn('Vídeo grande detectado, processamento pode demorar')
  }
  
  return {
    isValid: errors.length === 0,
    errors,
    warnings: file.size > 100 * 1024 * 1024 ? ['Arquivo grande, processamento pode demorar'] : []
  }
}
...
// Função para baixar arquivo - VERSÃO MELHORADA
...
// Função para verificar se o navegador suporta o tipo de arquivo
export const isFileTypeSupported = (file) => {
  const fileType = file.type.toLowerCase()
  
  // Verificar suporte nativo do navegador
  if (fileType.startsWith('video/')) {
  ...
  if (fileType.startsWith('image/')) {
    return true // Navegadores modernos suportam a maioria dos formatos de imagem
  }
  
  return true // Para outros tipos, assumir suporte
}
